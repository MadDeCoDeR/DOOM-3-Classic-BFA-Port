...
static HGLRC CreateOpenGLContextOnDC( const HDC hdc, const bool debugContext )
{
	...
	for( int i = 0; i < 2; i++ )
	{
		...
		if (!WGLEW_NV_DX_interop2 || useCoreProfile == 0)
		{
			m_hrc = wglCreateContext(hdc);
			break;
		}
...
{
	tempHwnd = CreateWindow(WIN32_FAKE_WINDOW_CLASS_NAME, ENGINE_NAME,
	...
	if( win32.hDC == NULL )
	{
		...
		if( ( win32.hDC = GetDC( tempHwnd ) ) == NULL )
		{
...
static bool InitDXGI(int w, int h, glimpParms_t parms) {
	ZeroMemory(&scd, sizeof(scd));
	scd.BufferCount = 1;
	scd.BufferDesc.Width = w;
	scd.BufferDesc.Height = h;
	scd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	scd.BufferDesc.RefreshRate.Numerator = parms.displayHz;
	scd.BufferDesc.RefreshRate.Denominator = 1;
	scd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
	scd.OutputWindow = win32.hWnd;
	scd.SampleDesc.Count = 1;
	scd.SampleDesc.Quality = 0;
	scd.Windowed = parms.fullScreen == 0;
