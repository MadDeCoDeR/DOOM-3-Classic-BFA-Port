...
public:
	...
	virtual void			Flush();
	// Seek on a file.
	virtual ID_INT				Seek(ID_LONG offset, fsOrigin_t origin );
...
private:
	...
	int						mode;			// open mode
	ID_INT					fileSize;		// size of the file
...
public:
	...
	virtual int				Tell() const;
	virtual ID_INT				Seek(uint64 offset, fsOrigin_t origin );
...
public:
	...
	virtual int				Tell() const;
	virtual ID_INT				Seek(ID_INT offset, fsOrigin_t origin );
...
private:
	idStr				name;				// name of the file in the pak
	ID_INT				offset;				// offset in the resource file
	ID_INT				length;				// size
	idFile* 			resourceFile;		// actual file
	ID_INT				internalFilePos;	// seek offset
