...
{
	if (extraammo) {	//SnoopJedi - Moved by deadite4 in 1.3.1
...
void idPlayer::RestorePersistantInfo()
{
	...
	extraammo = spawnArgs.GetBool("extraammo");
...
bool idPlayer::GivePowerUp( int powerup, int time, unsigned int giveFlags )
{
	...
	if( powerup >= 0 && powerup < CDMAX_POWERUPS )
	{
	
		if( giveFlags & ITEM_GIVE_UPDATE_STATE )
		{
			if (common->IsServer() && powerup != MEGAHEALTH && powerup != HEALTHVIAL && powerup != ARMORBONUS && powerup != MEGAARMOR)  // SnoopJeDi - LMS fix
				{
					idBitMsg	msg;
				...
				const idDeclEntityDef *def = NULL;
					...
					case HEALTHVIAL: {   //SnoopJeDi
						inventory.AddPickupName("Health Vial", this);
					...
					case ARMORBONUS: {  //SnoopJeDi
						inventory.AddPickupName("Armor Bonus", this);
					...
					case MEGAARMOR: {   //SnoopJeDi
						inventory.AddPickupName("Mega Armor", this);
...
void idPlayer::UpdatePowerupHud()
{
	...
	if( hudPowerup != -1 && hudPowerup < CDMAX_POWERUPS )
	{
...
void idPlayer::ReadFromSnapshot( const idBitMsg& msg )
{
	...
	*/
	...
	else if( health > oldHealth && PowerUpActive( MEGAHEALTH )  && PowerUpActive(HEALTHVIAL) && !snapshotStale )
	{
