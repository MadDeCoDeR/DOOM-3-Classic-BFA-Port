...
================
*/
void idTarget_EndLevelCdoom::Spawn() 
{
}
...
void idTarget_EndLevelCdoom::Event_Trigger(idEntity *activator) {
	...
	gameLocal->sessionCommand += idStr(spawnArgs.GetString("nextMap", ""));
}
...
void idTarget_Checkpoint::Event_Activate( idEntity* activator )
{
	...
	if( g_checkpoints.GetBool() && !g_demoMode.GetBool() )
	{
		...
		game->GetLocalPlayer()->ShowTip(idLocalization::GetString("#str_auto_save_title"), idLocalization::GetString("#str_auto_save_success"), true);														
	}
}
